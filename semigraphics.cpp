...
{
                        // single examples show below
                        //  v
    m_ssUL = ssUL;      // "╭" Upper left corner
    m_ssUR = ssUR;      // "╮" Upper right corner
    m_ssLL = ssLL;      // "╰" Lower left corner
    m_ssLR = ssLR;      // "╯" Lower right corner
    m_ssVSR = ssVSR;    // "├" Vertical splitter right
    m_ssVSL = ssVSL;    // "┤" Vertical splitter left
    m_ssHL = ssHL;      // "─" Horizontal line
    m_ssVL = ssVL;      // "│" Vertical line
}

/**
 * @brief Draws a single-line box at the specified starting position and
 * dimensions.
 *
 * This function creates a rectangular box made of single-line border
 * characters (such as ┌, ┐, ─, │, and ╰) on the terminal. The box is
 * drawn starting from the specified position and extends to the given
 * width and height.
 *
 * The box consists of:
 ...
 *
 * @param line The starting vertical coordinate (line) for the top-left
 * corner of the box.
 * @param col The starting horizontal coordinate (column) for the top-left
 * corner of the box.
 * @param width The overall width of the box, including the borders.
 * @param height The overall height of the box, excluding the height of the
 ...
 * Specifically, it draws a top border starting with "┌", followed by a
 * horizontal line "─" repeated across the specified width (adjusted for
 * the frame), and ends with the corner character "┐".
 *
 ...
 * @param height The total height of the box. This parameter is currently not
...
/**
 * @brief Renders the bottom line (endline) of a box or frame in semigraphics
 * style.
 *
 * This function outputs a decorative bottom line with a specific
 * semigraphics style. It starts with the "╰" character, followed
 * by a series of horizontal lines "─" that span from the given
 * starting column to the specified width. The line ends with "┘".
 *
 * @param line The starting line position (not utilized in this
 *          function).
 * @param col The starting column position on the console output.
 * @param width The total width of the box or frame.
 * @param height The total height of the box or frame (not utilized in this
 * function).
 *
 * @note This function assumes that the output stream is directed to a
 * console or terminal that supports and properly renders Unicode
