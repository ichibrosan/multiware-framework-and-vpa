...
void xinetcfg::initializeDefaultServices() {
    ...
    vpaService.wait = "no";
    vpaService.server = "/home/devo/public_html/fw/bin/xmlrpc_inetd_server";
    ...
    vpaNetstatService.server = "/home/devo/public_html/fw/scripts/inetd-netstat-redirect.sh";
    ...
    vpaIpService.server = "/home/devo/public_html/fw/scripts/inetd-ip-redirect.sh";
    ...
    vpaHttpService.server = "/home/devo/public_html/fw/scripts/inetd-http-redirect.sh";
    ...
    vpaHttpsService.server = "/home/devo/public_html/fw/scripts/inetd-https-redirect.sh";
    ...
    vpaIpcsService.server = "/home/devo/public_html/fw/scripts/inetd-ipcs-redirect.sh";
    vpaServices.push_back(vpaIpcsService);
}

std::string xinetcfg::formatServiceBlock(const XinetdServiceConfig& service) const {
...
std::string xinetcfg::generateConfigContent() const {
    ...
    config << "///////////////////////////////////////////////////\n";
    config << "// /etc/xinetd.d/vpa " << getCurrentDateTimeString() << " dwg -      //\n";
    ...
    // VPA main service
    config << "####################################################################" << std::endl;
    config << "# This service activates an xmlrpc responder to handle an incoming #" << std::endl;
    config << "# RPC request                                                      #" << std::endl;
    config << "####################################################################" << std::endl;
    ...
    // VPA daemon start service
    config << "###############################################################" << std::endl;
    config << "# This service runs a script that starts the virtual protocol #" << std::endl;
    config << "# adapter daemon                                              #" << std::endl;
    config << "###############################################################" << std::endl;
    ...
    // VPA netstat service
    config << "#################################################################" << std::endl;
    config << "# This service runs a script that calls the netstat utility and #" << std::endl;
    config << "# redirects output                                              #" << std::endl;
    config << "#################################################################" << std::endl;
    ...
    // VPA IP service
    config << "############################################################" << std::endl;
    config << "# This service runs a script that calls the ip utility and #" << std::endl;
    config << "# redirects output                                         #" << std::endl;
    config << "############################################################" << std::endl;
    ...
    // VPA HTTP service
    config << "##################################################################" << std::endl;
    config << "# This service runs a script that calls curl to load the default #" << std::endl;
    config << "# web page with http:// protocol                                 #" << std::endl;
    config << "##################################################################" << std::endl;
    ...
    config << "##################################################################" << std::endl;
    config << "# This service runs a script that calls curl to load the default #" << std::endl;
    config << "# web page with https:// protocol                                #" << std::endl;
    config << "##################################################################" << std::endl;
    ...
    // VPA IPCS service
    config << "########################################################################" << std::endl;
    config << "# This service runs a script that calls ipcs to show the shared status #" << std::endl;
    config << "# and redirects output                                                 #" << std::endl;
    config << "########################################################################" << std::endl;
...
void xinetcfg::displayVpaServiceStatus() {
    ...
    std::cout << "┌─────────────────┬───────┬──────────┬─────────────────────────────────────────────────────┐" << std::endl;
    std::cout << "│ Service Name    │ Port  │ Status   │ Server                                              │" << std::endl;
    std::cout << "├─────────────────┼───────┼──────────┼─────────────────────────────────────────────────────┤" << std::endl;
    
    for (const auto& service : vpaServices) {
        std::cout << "│ " << std::left << std::setw(15) << service.serviceName.substr(0, 15) << " │ ";
        std::cout << std::setw(5) << service.port << " │ ";
        
        std::string status = (service.disable == "yes") ? "DISABLED" : "ENABLED ";
    ...
    std::cout << "  vpad-start  (65353) - VPA daemon startup script" << std::endl;
    std::cout << "  vpa-netstat (65354) - Network status utility" << std::endl;
    std::cout << "  vpa-ip      (65355) - IP configuration utility" << std::endl;
    std::cout << "  vpa-http    (65357) - HTTP web page loader" << std::endl;
    std::cout << "  vpa-https   (65358) - HTTPS web page loader" << std::endl;
    std::cout << "  vpa-ipcs    (65359) - Shared memory status utility" << std::endl;
    std::cout << std::endl;
}
...
bool xinetcfg::saveConfig() {
    ...
    std::string tempFile = "/tmp/vpa_config_" + std::to_string(getpid()) + ".tmp";
    ...
    std::string command = "sudo cp '" + tempFile + "' '" + VPA_CONFIG_FILE + "' 2>/dev/null";
...
bool xinetcfg::loadConfig() {
    ...
    if (getuid() != 0) {
        ...
        std::string tempFile = "/tmp/vpa_read_" + std::to_string(getpid()) + ".tmp";
        std::string command = "sudo cp '" + VPA_CONFIG_FILE + "' '" + tempFile + "' 2>/dev/null";
...
bool xinetcfg::createBackup() const {
    if (getuid() == 0) {
        // Running as root
        std::string command = "cp '" + VPA_CONFIG_FILE + "' '" + VPA_CONFIG_BACKUP + "' 2>/dev/null";
        return system(command.c_str()) == 0;
    } else {
        // Use sudo
        std::string command = "sudo cp '" + VPA_CONFIG_FILE + "' '" + VPA_CONFIG_BACKUP + "' 2>/dev/null";
        return system(command.c_str()) == 0;
    }
}
...
bool xinetcfg::restoreFromBackup() const {
    if (getuid() == 0) {
        // Running as root
        std::string command = "cp '" + VPA_CONFIG_BACKUP + "' '" + VPA_CONFIG_FILE + "' 2>/dev/null";
        return system(command.c_str()) == 0;
    } else {
        // Use sudo
        std::string command = "sudo cp '" + VPA_CONFIG_BACKUP + "' '" + VPA_CONFIG_FILE + "' 2>/dev/null";
        return system(command.c_str()) == 0;
    }
}
...
bool xinetcfg::reloadXinetd() const {
    ...
    std::string command = "sudo systemctl reload xinetd 2>/dev/null || sudo service xinetd reload 2>/dev/null";
    return system(command.c_str()) == 0;
}

bool xinetcfg::restartXinetd() const {
    // Always use sudo for service operations
    std::string command = "sudo systemctl restart xinetd 2>/dev/null || sudo service xinetd restart 2>/dev/null";
    return system(command.c_str()) == 0;
}

bool xinetcfg::isXinetdRunning() const {
    // Check service status (no sudo needed for status check)
    std::string command = "systemctl is-active xinetd 2>/dev/null || service xinetd status 2>/dev/null";
    return system(command.c_str()) == 0;
}
...
void xinetcfg::displaySudoInstructions() const {
    ...
    std::cout << "To modify /etc/xinetd.d/vpa, you need root privileges." << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "   Add: " << getCurrentUser() << " ALL=(ALL) NOPASSWD: /bin/cp, /bin/chmod, /usr/bin/systemctl" << std::endl;
    std::cout << std::endl;
...
size_t xinetcfg::parseServiceBlock(const std::vector<std::string>& lines, size_t startIndex, XinetdServiceConfig& service) const {
...
bool xinetcfg::parseExistingConfig() {
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        if (trimmedLine.find("service ") == 0) {
            ...
            service.serviceName = trimmedLine.substr(nameStart, nameEnd - nameStart);
