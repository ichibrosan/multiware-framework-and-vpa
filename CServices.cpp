...
CServices::CServices() : cacheLoaded(false) {
    ...
    if (!isSudoAvailable()) {
        std::cerr << "Warning: sudo not available or not configured properly." << std::endl;
...
bool CServices::parseServiceLine(const std::string& line, ServiceInfo& service) const {
...
bool CServices::hasService(const std::string& serviceName, const std::string& protocol) const {
    loadServicesCache();
...
ServiceInfo CServices::getService(const std::string& serviceName, const std::string& protocol) const {
    loadServicesCache();
...
ServiceInfo CServices::getServiceByPort(int port, const std::string& protocol) const {
    loadServicesCache();
...
bool CServices::addService(const std::string& serviceName, int port, const std::string& protocol,
                          const std::vector<std::string>& aliases) {
    // Validate inputs
    if (!isValidServiceName(serviceName) || !isValidProtocol(protocol) || port < 1 || port > 65535) {
        return false;
    }
    ...
    if (!createBackup()) {
        std::cerr << "Warning: Could not create backup of " << SERVICES_FILE << std::endl;
    }
...
bool CServices::removeService(const std::string& serviceName, const std::string& protocol) {
...
std::vector<ServiceInfo> CServices::findServices(const std::string& pattern) const {
    loadServicesCache();
...
int CServices::getServicePort(const std::string& serviceName, const std::string& protocol) const {
    ServiceInfo service = getService(serviceName, protocol);
    return service.name.empty() ? -1 : service.port;
}

std::string CServices::getServiceName(int port, const std::string& protocol) const {
...
void CServices::displaySudoInstructions() const {
    ...
    std::cout << "To use CServices for /etc/services management, you need to configure sudo." << std::endl;
    std::cout << "As root, add this line to /etc/sudoers (use 'visudo' command):" << std::endl;
    std::cout << std::endl;
    std::cout << user << " ALL=(ALL) NOPASSWD: /bin/cp /tmp/services_* /etc/services" << std::endl;
    std::cout << user << " ALL=(ALL) NOPASSWD: /bin/cp /etc/services /etc/services.backup" << std::endl;
    std::cout << user << " ALL=(ALL) NOPASSWD: /bin/cp /etc/services.backup /etc/services" << std::endl;
    std::cout << user << " ALL=(ALL) NOPASSWD: /usr/bin/systemctl reload xinetd" << std::endl;
    std::cout << user << " ALL=(ALL) NOPASSWD: /usr/sbin/service xinetd reload" << std::endl;
    std::cout << user << " ALL=(ALL) NOPASSWD: /etc/init.d/xinetd reload" << std::endl;
    std::cout << user << " ALL=(ALL) NOPASSWD: /usr/bin/killall -HUP xinetd" << std::endl;
    std::cout << std::endl;
    std::cout << "Or for broader access (less secure):" << std::endl;
    std::cout << user << " ALL=(ALL) NOPASSWD: /bin/cp, /usr/bin/systemctl, /usr/sbin/service, /etc/init.d/xinetd, /usr/bin/killall" << std::endl;
    std::cout << "===================================" << std::endl;
}
...
std::string CServices::createTempFile(const std::string& content) const {
    std::string tempFile = TEMP_DIR + "/services_" + std::to_string(getpid()) + "_" + std::to_string(time(nullptr));
